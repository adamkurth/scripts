...
# Define directories
PDB_DIR="$(pwd)/$SPACEGROUP/pdb/pdb_$SPACEGROUP"
TXT_OUTDIR="$(pwd)/$SPACEGROUP/out/out_$SPACEGROUP"
MTZ_OUTDIR="$(pwd)/$SPACEGROUP/mtz/mtz_$SPACEGROUP"
HKL_OUTDIR="$(pwd)/$SPACEGROUP/data/data_$SPACEGROUP"

# Ensure the PDB directory exists
if [ ! -d "$PDB_DIR" ]; then
    read -p "PDB directory $PDB_DIR does not exist. Do you want to create it? (y/n): " choice
    if [ "$choice" == "y" ]; then
        mkdir -p "$PDB_DIR"
        echo "PDB directory $PDB_DIR created."
    else
        echo "PDB directory $PDB_DIR does not exist. Exiting..."
        exit 1
    fi
fi
